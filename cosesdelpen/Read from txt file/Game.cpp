...
//Vertex data[MAX_VER..]->vertex * data in the game class
//Read and store the information into the DS
//Update the drawGame method
...
void Game::createPrimitivesToRender() {
	...
	myArray.CreateArray();
	//cout << myArray.returnData(1) << endl;
	for (int i = 0; i < myArray.getVertices(); i++){
		data[i].setPosition(myArray.returnData(i*7), myArray.returnData(i*7+1), myArray.returnData(i*7+2));
	}
	////First triangle
	...
	for (int i = 0; i < myArray.getVertices(); i++) {
		...
		data[i].setColor(myArray.returnData(i*7+3), myArray.returnData(i*7+4), myArray.returnData(i*7+5), myArray.returnData(i*7+6));
	}
